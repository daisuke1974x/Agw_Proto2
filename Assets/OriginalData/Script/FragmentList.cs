...
public class FragmentList : MonoBehaviour
{
        ...
        if (Input.GetButtonDown("Circle"))
        {
            if (0<StockList.Count)
            {
                if (MainControl.GetComponent<MainControl>().isIdle == true)
                {
                    ...
                    MainCamera.GetComponent<MainCamera>().isControllEnabled = false;
    ...
    void Update_TransitionLargeStockSetToStockList()
    {
        ...
        if (TimeStep1 < TimerCounter)
        {
            ...
            MainCamera.GetComponent<MainCamera>().isControllEnabled = true;
        }
    ...
    void Update_LargeStockHandleRotate()
    {
        ...
        Vector3 RotateCenter = MainControl.GetComponent<MainControl>().FieldCursorObject.transform.position;//回転の中心の指定
